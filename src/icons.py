...
The design system mandates Lucide 0.454.0 at stroke-width 1.75. Streamlit strips
`<script>` from injected HTML, so the CDN hydration path is unavailable; the
inner markup below is copied **verbatim** from `lucide-static@0.454.0` (ISC
licensed) and only the wrapper attributes (size, stroke-width) are set here.
Do not hand-edit the path data — re-copy from the package if a glyph is added.
"""
...
_PATHS: dict[str, str] = {
    ...
    "activity": '<path d="M22 12h-2.48a2 2 0 0 0-1.93 1.46l-2.35 8.36a.25.25 0 0 1-.48 0L9.24 2.18a.25.25 0 0 0-.48 0l-2.35 8.36A2 2 0 0 1 4.02 12H2"/>',
    "thermometer": '<path d="M14 4v10.54a4 4 0 1 1-4 0V4a2 2 0 0 1 4 0Z"/>',
}

# Semantic aliases used by the verdict components.
_ALIASES = {
    "good": "circle-check",
    "bad": "triangle-alert",
...
def icon(name: str, size: int = 16, *, cls: str = "", stroke: float = 1.75) -> str:
    """Return an inline `<svg>` string for `name` at the given pixel size."""
...
def has(name: str) -> bool:
    return name in _PATHS or name in _ALIASES
